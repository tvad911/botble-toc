...
class TocHelper
{
    ...
    public function replaceShortcode($content)
    {
        ...
        $content = str_replace($this->noTocShortcode, $this->noTocTag, $temp1);
    ...
    public function removeTag($content)
    {
        ...
        $content = str_replace($this->noTocTag, '', $temp1);
     ...
     * @param string $content
     */
    public function theContentShortcode($content, $object)
    {
        $content = $this->replaceShortcode($content);
        $customTocPosition = strpos($content, $this->theTag);
        $find = [];
        $replace = [];
        $items = $this->extractHeadings($find, $replace, $content);
        $checkRemoveToc = strpos($content, $this->noTocTag);
